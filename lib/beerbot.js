...
BeerBot.prototype.getChannel = function(channel) {
  return this.slack.getChannelGroupOrDMByID(channel) || this.slack.getChannelGroupOrDMByName(channel);
};

BeerBot.prototype.send = function(channel, message) {
  if (this.options.silent) {
    return this.logger.info('Silent Mode: Push message on channel %s: "%s"', channel, message);
  }

  var slackChannel = this.getChannel(channel);
  if (!slackChannel) {
...
BeerBot.prototype.sendRichText = function(channel, message) {
  var slackChannel = this.getChannel(channel);
  if (!slackChannel) {
    return this.logger.error('Can not find channel %s', channel);
  }
  slackChannel.postMessage({text: message, unfurl_links: true});
};
...
BeerBot.prototype.initialize = function() {
  ...
  self.slack.on('message', function(message) {
    ...
    if (type === 'message' && text && channel) {
      ...
      self.logger.debug('Can not handle message type %s with text %s', type, text);
    }
  });
};
